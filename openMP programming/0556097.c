...
//#pragma omp parallel
  ...
  //#pragma omp parallel
    ...
    for (j = 0; j < lastrow - firstrow + 1; j++) {
      sum = 0.0;
			//#pragma omp parallel for reduction(+:sum) shared(j)
      for (k = rowstr[j]; k < rowstr[j+1]; k++) {
...
//#pragma omp parallel    //why collapse(2) dosent works?? try collapes(2) shared(i)?dosent work too
  //#pragma omp single
  for (i = 0; i < n; i++) {
  ...
  for (j = 1; j < nrows+1; j++) {
    rowstr[j] = rowstr[j] + rowstr[j-1]; // loop-carried dependency??
  }

  nza = rowstr[nrows] - 1;

  //---------------------------------------------------------------------
  // ... rowstr(j) now is the location of the first nonzero1
  //     of row j of a
  //---------------------------------------------------------------------
  if (nza > nz) {
...
//loop-carried dependency?
  ...
  for (j = 0; j < nrows; j++) {
    ...
    nza = rowstr[j];
		//#pragma omp parallel for lastprivate(nza)
    for (k = j1; k < j2; k++) {
      //nza = rowstr[j] + (j1-k);
      a[k] = a[nza];
      colidx[k] = colidx[nza];
