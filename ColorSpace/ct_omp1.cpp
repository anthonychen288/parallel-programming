...
void sdm(BMP img, float *Msd)
{
	...
	#pragma omp parallel
	{
		...
		double loc_mean[3] = {0.0};
		//printf("%d \n", tid);
	for(int i=tid;i<height;i+=thread_count)
	{
		for(int j=0;j<width;j++)
		{
			...
			loc_mean[0] += mr[i * height + j];
			loc_mean[1] += mg[i * height + j];
			loc_mean[2] += mb[i * height + j];
		}
		
	}
	//printf("thread %d %f %f %f\n", tid, loc_mean[0], loc_mean[1], loc_mean[2]);
	#pragma omp atomic
	Msd[0] += loc_mean[0];
	#pragma omp atomic
	Msd[1] += loc_mean[1];
	...
	for(int i=tid;i<height;i+=thread_count)
	{
		for(int j=0;j<width;j++)
		{
			loc_sd[0] = loc_sd[0]+(mr[i * height + j] - Msd[0])*(mr[i * height + j] - Msd[0]);
			loc_sd[1] = loc_sd[1]+(mg[i * height + j] - Msd[1])*(mg[i * height + j] - Msd[1]);
			loc_sd[2] = loc_sd[2]+(mb[i * height + j] - Msd[2])*(mb[i * height + j] - Msd[2]);
		}
	}
	//printf("thread %d %f %f %f\n", tid, loc_sd[0], loc_sd[1], loc_sd[2]);
	#pragma omp atomic
	Msd[3] += loc_sd[0];
	#pragma omp atomic
	Msd[4] += loc_sd[1];
