...
/* These functions are defined in MyBMP.h */

bool MyBMPwarnings = true;
...
{ return MyBMPwarnings; }

/* These functions are defined in MyBMP_DataStructures.h */
...
/* These functions are defined in MyBMP_BMP.h */
...
RGBApixel BMP::GetPixel( int i, int j ) const
{
 ...
 if( j < 0 )
 { j = 0; Warn = true; }
 if( Warn && MyBMPwarnings )
 {
  cout << "MyBMP Warning: Attempted to access non-existent pixel;" << endl
...
bool BMP::SetColor( int ColorNumber , RGBApixel NewColor )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: Attempted to change color table for a BMP object" << endl
 ...
 if( !Colors )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: Attempted to set a color, but the color table" << endl
 ...
 if( ColorNumber >= TellNumberOfColors() )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: Requested color number "
...
RGBApixel BMP::GetColor( int ColorNumber )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: Attempted to access color table for a BMP object" << endl
        << "                 that lacks a color table. Ignoring request." << endl;
  }
  return Output;
 }
 if( !Colors )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: Requested a color, but the color table" << endl
        << "                 is not defined. Ignoring request." << endl;
  }
  return Output;
 }
...
bool BMP::SetBitDepth( int NewDepth )
{
     ...
     NewDepth != 24 && NewDepth != 32 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: User attempted to set unsupported bit depth "
        << NewDepth << "." << endl
...
bool BMP::SetSize(int NewWidth , int NewHeight )
{
 ...
 if( NewWidth <= 0 || NewHeight <= 0 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: User attempted to set a non-positive width or height." << endl
...
{
 using namespace std;
 if( !MyBMPcheckDataSize() )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: Data types are wrong size!" << endl
        << "               You may need to mess with MyBMP_DataTypes.h" << endl
 ...
 if( fp == NULL )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: Cannot open file "
 ...
 if( BitDepth != 16 )
 {
  ...
  while( j > -1 )
  {
   ...
   if( !Success )
   {
    if( MyBMPwarnings )
    {
     cout << "MyBMP Error: Could not write proper amount of data." << endl;
	}
    j = -1;
   }
   j--;
...
bool BMP::ReadFromFile( const char* FileName )
{
 ...
 if( bmih.biCompression == 1 || bmih.biCompression == 2 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: " << FileName << " is (RLE) compressed." << endl
        << "               MyBMP does not support compression." << endl;
  }
  SetSize(1,1);
  SetBitDepth(1);
  fclose(fp);
 ...
 if( bmih.biCompression > 3 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: " << FileName << " is in an unsupported format."
        << endl
        << "               (bmih.biCompression = "
     ...
     && TempBitDepth != 24 && TempBitDepth != 32 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: " << FileName << " has unrecognized bit depth." << endl;
  }
  SetSize(1,1);
  SetBitDepth(1);
  fclose(fp);
 ...
 if( BitDepth < 16 )
 {
  ...
  if( NumberOfColorsToRead < TellNumberOfColors() )
  {
   if( MyBMPwarnings )
   {
    cout << "MyBMP Warning: file " << FileName << " has an underspecified" << endl
 ...
 if( BytesToSkip > 0 && BitDepth != 16 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: Extra meta data detected in file " << FileName << endl
 ...
 if( BitDepth != 16 )
 {
  ...
  while( j > -1 )
  {
   ...
   if( BytesRead < BufferSize )
   {
    ...
    if( MyBMPwarnings )
    {
     cout << "MyBMP Error: Could not read proper amount of data." << endl;
	}
   }
   else
   {
    ...
    if( !Success )
    {
     if( MyBMPwarnings )
     {
      cout << "MyBMP Error: Could not read enough pixel data!" << endl;
	 }
	 j = -1;
    }
   }
 ...
 if( BitDepth == 16 )
 {
  ...
  if( BytesToSkip > 0 )
  {
   if( MyBMPwarnings )
   {
    cout << "MyBMP Warning: Extra meta data detected in file "
         << FileName << endl
         << "                 Data will be skipped." << endl;
   }
   ebmpBYTE* TempSkipBYTE;
...
bool BMP::CreateStandardColorTable( void )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Warning: Attempted to create color table at a bit" << endl
 ...
 if( BitDepth == 8 )
 {
  ...
  // do an My loop, which works for all but colors
...
/* These functions are defined in MyBMP_VariousBMPutilities.h */
...
BMFH GetBMFH( const char* szFileNameIn )
{
 ...
 if( !fp  )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: Cannot initialize from file "
        << szFileNameIn << "." << endl
...
bool MyBMPcheckDataSize( void )
{
 ...
 if( sizeof( ebmpBYTE ) != 1 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: ebmpBYTE has the wrong size ("
 ...
 if( sizeof( ebmpWORD ) != 2 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: ebmpWORD has the wrong size ("
 ...
 if( sizeof( ebmpDWORD ) != 4 )
 {
  if( MyBMPwarnings )
  {
   cout << "MyBMP Error: ebmpDWORD has the wrong size ("
...
bool Rescale( BMP& InputImage , char mode, int NewDimension )
{
     ...
     CapMode != 'F' )
 {
  if( MyBMPwarnings )
  {
   ...
   sprintf( ErrorMessage, "MyBMP Error: Unknown rescale mode %c requested\n" , mode );
   cout << ErrorMessage;
  }
  return false;
 }
